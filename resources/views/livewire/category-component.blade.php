<div class="wrapper">
   ...
   <div class="container flex flex-col justify-center items-center gap-y-12">
      ...
      <div class="w-full grid s:grid-cols-2 lg:grid-cols-3 gap-y-12 gap-x-7">
         ...
         <x-link href="{{$product->route()}}" title="{{$product->name}}" class="block space-y-7 group">
            <div class="aspect-w-3 aspect-h-2 relative overflow-hidden">
               <x-image src="{{$product->image}}" alt="{{$product->name}}" width="100%" height="100%" class="size-full object-contain transition-transform duration-300 group-hover:scale-110" />
            </div>
